...
class EmbeddingService:
    ...
    def embed_text(self, text: str) -> List[float]:
        ...
        client = self._get_openai_client()
        try:
            kwargs = {"model": self.embedding_config.model_name, "input": text[:8000]}
            if self.embedding_config.model_name.startswith("text-embedding-3"):
                kwargs["dimensions"] = int(self.embedding_config.embedding_dim)
    ...
    def embed_texts_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                kwargs = {"model": self.embedding_config.model_name, "input": batch}
                if self.embedding_config.model_name.startswith("text-embedding-3"):
                    kwargs["dimensions"] = int(self.embedding_config.embedding_dim)
                response = client.embeddings.create(**kwargs)
